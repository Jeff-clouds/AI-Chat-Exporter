...
### 1.2 ChatGPT 重页面的轻量核查策略

ChatGPT Web 端完整 DOM 树读取经常因页面规模、异步渲染和控制通道负载而超时。完整 snapshot 不应作为日常核查入口，也不能因 snapshot 超时就判断 selector 漂移或插件失效。

核查应逐层增加信息量：

1. 先确认当前 URL、页面加载状态和会话类型；
2. 再用小范围运行时探针统计 turn、message identity、角色和滚动容器，不读取会话正文；
3. 只在定位具体矛盾时读取单个目标节点及其有限祖先/后代；
4. 目录跳转以点击后的目标身份、可见位置和侧栏回执为验收，不要求导出整棵 DOM；
5. 浏览器控制超时单独记录为“本轮无法验证”，不得升级为宿主变化或代码故障。

轻量探针仍然只是特定日期与样本的现场证据，不能被固化成永久 selector 合约。
