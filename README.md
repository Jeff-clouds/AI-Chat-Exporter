...
- **Pro 多格式导出**：完整或局部对话均可选择 HTML、JSON、TXT 格式。
- **按需运行**：仅在侧边栏打开或用户主动导出时分析页面，关闭侧边栏后停止持续监听与索引。
...
4. 选择导出格式；Markdown 对所有用户免费，HTML、JSON、TXT 由 Pro 解锁。
5. 点击“导出完整对话”保存文件。Pro 用户也可进入选择模式，只导出勾选的问题组。
...
- `src/export/`：Markdown、HTML、JSON、TXT 生成和文件下载
